...
spec = [("v0",float64),("mu",float64),("Frep",float64),("R0",float64),("x",float64),("y",float64),("vx",float64),("vy",float64),("ident",int64),("Raio_equilibrio",float64),("fadh",float64),("noise",float64),("r",float64[:]),("v",float64[:]),("n",float64[:]),("theta",float64),("Mybox",int64),("Force",float64[:]),("Req",float64),("cross",int64),("cross2",int64),("Fadh",float64),("f",float64),("dr",float64),("normdr",float64),]
...
spec2 = [("index",int64),("mylist",types.ListType(int64)),("neighboxlist",types.ListType(int64)),]
@NB.njit
...
class boite:
    def __init__(self,index):
        self.index = index
        self.mylist = typed.List.empty_list(types.int64)
        self.neighboxlist = typed.List.empty_list(types.int64)

    # def neighbor_list(self):
    #     self.neighboxlist = typed.List.empty_list(types.int64)#empty_int64_list()
    #     if self.index%nb[0]!=nb[0]-1:                                #test if not last column
    #         print(box[self.index+1].mylist)
    ...
    #         print(box[self.index+nb[0]-1].mylist)
    ...
    #         print(box[self.index+nb[0]].mylist)
...
def neigh():
    for i in range(nb2):
        box[i].neighboxlist = typed.List.empty_list(types.int64)
        if i%nb[0]!=nb[0]-1:           #test if not last column
            box[i].neighboxlist.extend(box[i+1].mylist)
        if i%nb[0]!=0 and i+nb[0]<nb2: #test if not first column and not last line
            box[i].neighboxlist.extend(box[i+nb[0]-1].mylist) #add left box list below
        if i+nb[0]<nb2:            #test if not last line
            box[i].neighboxlist.extend(box[i+nb[0]].mylist)   #add central box list below
        if i+nb[0]<nb2 and i%nb[0]!=nb[0]-1:       #test if not last line and last column
            box[i].neighboxlist.extend(box[i+nb[0]+1].mylist) #add right box list below
    return
...
# for i in range(nb2):
#     if box[i].neighboxlist:
#         print(box[i].neighboxlist)
